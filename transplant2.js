...
// Get the original HTML from git securely using child_process (avoids powershell encoding issues)
const oldHtml = execSync('git show 06eeec6:website/index.html', { encoding: 'utf8' });

// We already committed the newHtml today, so let's get the version of newHtml before the garble.
// The current website/index.html is garbled. 
// I can get the pristine new HTML from HEAD (since I committed it earlier).
const newHtml = execSync('git show HEAD:website/index.html', { encoding: 'utf8' });
...
let finalHtml = oldHtml.slice(0, oldPackagesStart) + 
  ...
  '<button id="currencyEGP" class="btn" style="border-radius: 25px; background: var(--yellow); color: #000; padding: 8px 24px; border:none;" onclick="setCurrency(\'egp\')">???? EGP</button>' +
  '<button id="currencyUSD" class="btn" style="border-radius: 25px; background: transparent; color: #fff; padding: 8px 24px; border:none;" onclick="setCurrency(\'usd\')">?? USD</button>' +
  '</div></div>\n' +
  newPricing + 
  '\n</div></section>\n\n' +
...
console.log("Transplanted properly this time!");
