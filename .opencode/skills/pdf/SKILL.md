...
# Process PDFs via CLI
python scripts/pdf.py extract text document.pdf
python scripts/pdf.py pages merge a.pdf b.pdf -o merged.pdf
```

**Python wrapper** (importable by the project):
```python
from pdf_skill import (
    merge_pdfs, extract_text, compress_pdf, html_to_pdf, ...
)
```

**Exit codes:**
- `0` = success, `1` = usage error, `2` = dependency missing, `3` = runtime error
...
- Use standard structures based on document type:
  ...
  - **Business reports**: Conclusion-first
...
- `pdf.sh` — Unified CLI (check/fix/html/latex/process)
- `pdf.py` — Python CLI entry point for PDF processing
- `html_to_pdf.js` — HTML → PDF conversion via Playwright + Paged.js
- `browser_helper.js` — Shared Chromium browser detection
- `setup.sh` — Environment diagnostics

## Python Wrapper

The module `pdf_skill.py` provides a clean, importable Python API:
```python
from pdf_skill import (
    merge_pdfs, split_pdf, rotate_pdf, crop_pdf,
    extract_text, extract_tables, extract_images,
    get_metadata, set_metadata, fill_form, get_form_fields,
    compress_pdf, html_to_pdf
)
```
