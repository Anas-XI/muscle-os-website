...
numbersHtml += '<div class="dash-macro-grid">' +
 ...
 renderFoodTab();
  ...
  window.showTourStep = function(stepIdx){
    if(stepIdx < 0 || stepIdx >= MOS_TOUR_STEPS.length) return;
    ...
    // Smoothly transition the user directly to the section explained
    try {
      if(stepIdx === 0){
        var profileCard = document.getElementById('profileFormCard');
        if(profileCard){
          profileCard.style.display = 'block';
          var setupTab = document.querySelector('#profileFormCard .tab[data-tab="tab-setup"]');
          if(setupTab) setupTab.click();
        }
        window.scrollTo({top: 0, behavior: 'smooth'});
      } else if(stepIdx === 1){
        if(!store.profile){
          store.profile = { sex: 'male', weight: 75, height: 175, age: 25, bf: 15, trainingAge: 'intermediate', trainDays: 4, sessionLen: 60, occupation: 'desk', steps: 8000, sleep: 8, dietPattern: 'standard', goal: 'moderate_loss', chronicDiet: 'no', cyclePhase: 'follicular' };
          saveStore();
        }
        var pView = document.getElementById('profileView'); if(pView) pView.style.display = 'none';
        var dView = document.getElementById('dashboardView'); if(dView) dView.style.display = 'block';
        if(typeof renderDashboard === 'function') renderDashboard();
        var foodTabBtn = document.querySelector('#dashboardView .tab[data-tab="dash-food"]');
        if(foodTabBtn) foodTabBtn.click();
        if(typeof renderFoodTab === 'function') renderFoodTab();
        window.scrollTo({top: 0, behavior: 'smooth'});
      } else if(stepIdx === 2){
        var leuSection = document.getElementById('leucineTracker') || document.querySelector('.mcu-box') || document.getElementById('microNutrientGrid');
        if(leuSection) leuSection.scrollIntoView({behavior: 'smooth', block: 'center'});
      } else if(stepIdx === 3){
        var carbSection = document.querySelector('.carb-cycling-bar') || document.getElementById('carbCycleContainer') || document.querySelector('.chronotype-bar');
        if(carbSection) carbSection.scrollIntoView({behavior: 'smooth', block: 'center'});
      } else if(stepIdx === 4){
        var revSection = document.getElementById('reverseDietSection') || document.getElementById('groceryListBtn') || document.querySelector('.productivity-suite');
        if(revSection) revSection.scrollIntoView({behavior: 'smooth', block: 'center'});
      }
    } catch(err){
    ...
    modal.innerHTML = `
      <div class="card" style="max-width:480px;width:92%;border:1.5px solid #262936;box-shadow:0 20px 50px rgba(0,0,0,0.85), 0 0 24px rgba(226,232,240,.18);background:rgba(22,24,34,0.96);backdrop-filter:blur(16px);-webkit-backdrop-filter:blur(16px);border-radius:16px;padding:20px;">
        <div style="font-family:'Oswald',sans-serif;font-size:1.15rem;color:#E2E8F0;margin-bottom:8px;display:flex;align-items:center;justify-content:space-between;">
          ...
          <span style="font-family:'Inter',sans-serif;font-size:0.65rem;color:rgba(250,250,248,0.4);font-weight:600;">Metabolix Guide</span>
        </div>
        <div style="font-size:.78rem;line-height:1.6;color:rgba(250,250,248,.88);margin-bottom:16px;">${step.desc}</div>
        <div style="display:flex;justify-content:space-between;align-items:center;">
          <span style="font-size:.65rem;color:rgba(250,250,248,.7);font-weight:600;">Step ${stepIdx + 1} of ${MOS_TOUR_STEPS.length}</span>
          <div style="display:flex;gap:6px;">
            ${stepIdx > 0 ? `<button class="btn-secondary" onclick="showTourStep(${stepIdx - 1})" style="padding:6px 12px;font-size:.7rem;">◄ Back</button>` : ''}
            ${stepIdx < MOS_TOUR_STEPS.length - 1 ? `<button class="btn-primary" onclick="showTourStep(${stepIdx + 1})" style="margin:0;padding:6px 14px;font-size:.7rem;">Next ➔</button>` : `<button class="btn-primary" onclick="closeTourModal()" style="margin:0;padding:6px 14px;font-size:.7rem;">Finish Tour ✓</button>`}
            <button class="btn-secondary" onclick="closeTourModal()" style="padding:6px 10px;font-size:.7rem;">Skip</button>
          </div>
        </div>
      </div>
    `;
