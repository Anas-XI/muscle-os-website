...
from mos_os.core.intake_builder import (
...
try:
    from mos_os.core.supabase_sync import fire_push_profile
except Exception:
    def fire_push_profile(*a, **kw): pass
from mos_os.config import OWNER_ID
try:
...
def _crisis_resources_text() -> str:
    from mos_os.core.context_loader import format_crisis_resources
...
async def confirm_handler(update, context):
    ...
    if profile.get("mental_health_concern") == "moderate":
        try:
            from mos_os.core.mental_health_flags import create_or_trigger_flag
