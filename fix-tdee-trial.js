...
// 1. Inject trial logic before `if(!active){`
const trialLogic = `
  // TDEE Trial Logic
  var TRIAL_DAYS = 7;
...
content = content.replace("if(!active){", trialLogic);

// We need to close the `} else {` at the end of the `if(!active)` block.
// The end of `if(!active)` block has:
//    showStep(2);
//    });
//    }
//    }
...
console.log("Injected TDEE Trial logic.");
