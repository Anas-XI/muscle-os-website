...
(function () {
  ...
  function getWorkerBase() {
    ...
    if (window.__MOS_CONFIG__ && window.__MOS_CONFIG__.API_BASE) return window.__MOS_CONFIG__.API_BASE;
    return 'https://muscleos-access-control.muscleos.workers.dev/api';
  }

  function getToken() {
    try {
      var s = localStorage.getItem('mos_subscription');
      if (s) {
        var p = JSON.parse(s);
        if (p && p.token) return p.token;
      }
      return localStorage.getItem('mos_session');
    } catch (e) { return null; }
  }
  ...
  function enqueueSync(item) {
    ...
    // Register BackgroundSync with ServiceWorker if supported
    if ('serviceWorker' in navigator && 'SyncManager' in window) {
      navigator.serviceWorker.ready.then(function(reg) {
        return reg.sync.register('mos-sync-queue');
      }).catch(function(err) {
        console.warn('[SyncManager] BackgroundSync registration skipped:', err);
      });
    }
  }
  ...
  if ('serviceWorker' in navigator) {
    navigator.serviceWorker.addEventListener('message', function(event) {
      if (event.data && event.data.type === 'MOS_SYNC_TRIGGER') {
        console.info('[SyncManager] BackgroundSync wakeup received — flushing sync queue...');
        flushSyncQueue();
      }
    });
  }
